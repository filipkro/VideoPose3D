...
def fetch_orig(subjects, action_filter=None, subset=1, parse_3d_poses=True):
    out_poses_3d = []
    out_poses_2d = []
    out_camera_params = []
    for subject in subjects:
        for action in keypoints[subject].keys():
            ...
            print('poses_2d ([subject][action]): {}'.format(poses_2d[0].shape))
...
def fetch(subjects, action_filter=None, subset=1, parse_3d_poses=True):
    ...
    print('poses 3d: {}'.format(out_poses_3d[0].shape))
    print('poses 2d: {}'.format(out_poses_2d))
    print('out camera: {}'.format(out_camera_params))
...
if not args.disable_optimizations and not args.dense and args.stride == 1:
    ...
    model_pos_train = TemporalModelOptimized1f(poses_valid_2d[0][0].shape[-2], poses_valid_2d[0][0].shape[-1], outjoints,
    ...
    model_pos_train = TemporalModel(poses_valid_2d[0].shape[-2], poses_valid_2d[0].shape[-1], outjoints,
                                    filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels,
                                    dense=args.dense)

model_pos = TemporalModel(poses_valid_2d[0][0].shape[-2], poses_valid_2d[0][0].shape[-1], outjoints,
                          filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels,
                          dense=args.dense)
# model_pos_train.load_state_dict(loaded_weights['model_pos'])
...
test_generator = UnchunkedGenerator(cameras_valid, poses_valid, poses_valid_2d[0],
...
if not args.evaluate:
    ...
    train_generator = ChunkedGenerator(args.batch_size // args.stride, cameras_train, poses_train, poses_train_2d[0], args.stride,
                                       pad=pad, causal_shift=causal_shift, shuffle=True, augment=args.data_augmentation,
                                       kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right)
    train_generator_eval = UnchunkedGenerator(cameras_train, poses_train, poses_train_2d[0],
                                              pad=pad, causal_shift=causal_shift, augment=False)
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            epoch_loss_3d_pos_scale += inputs_3d.shape[0] * inputs_3d.shape[1] * n_mpjpe(
                predicted_3d_pos, inputs_3d).item()
...
if args.render:
    ...
    def fetch_actions_orig(actions):
        out_poses_3d = []
        out_poses_2d = []
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            gen = UnchunkedGenerator(None, poses_act, poses_2d_act[0],
                                     pad=pad, causal_shift=causal_shift, augment=args.test_time_augmentation,
                                     kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right)

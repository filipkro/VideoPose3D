...
def fetch_custom(subjects, action_filter=None, subset=1, parse_3d_poses=True):
    out_poses_3d = []
    out_poses_2d = []
    out_camera_params = []
    for subject in subjects:
        ...
        for action in dataset[subject].keys():
            ...
            # poses_2d = keypoints[subject][action]
            poses_2d = dataset[subject][action]['positions_2d']
            print('poses_2d ([subject][action]): {}'.format(poses_2d[0].shape))
            for i in range(len(poses_2d)):  # Iterate across cameras
                out_poses_2d.append(poses_2d[i])
...
print('poses_valis', poses_valid)
print('pose 2d', poses_valid_2d)
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            if not use_trajectory_model:
                print('before:::')
                print(inputs_2d)
                print('2d shape', inputs_2d.size())
                print(inputs_2d[0, ...])
                print(inputs_2d[1, ...])
                print('diff', inputs_2d[0, ...] - inputs_2d[1, ...])
                predicted_3d_pos = model_pos(inputs_2d)
...
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            print('pose sahape far down', poses_2d_act.shape)
